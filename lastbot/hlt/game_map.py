...
class GameMap:
    ...
    def convolveMax(self,kernelSize = 8, padding = 0, stride = 4):
        ...
        topAverage = topAverage/8

        haliteSort1 = [(convolvedMaxPos[i],i) for i in range(len(convolvedMaxPos))]
        def calculate_position(a):
            return Position(a[0][0]*stride+kernelSize//2,a[0][1]*stride+kernelSize//2)

        disSort2 = sorted(haliteSort1,key = lambda x: abs(self.width*0.5-self.calculate_distance(calculate_position(x),self.game.me.shipyard.position)))
        ...
        return calculate_position(average[0]),[calculate_position(average[i]) for i in range(len(average))],topAverage
    ...
    def naive_navigate(self, ship, destination,maxHalite = False,finalReturn = False, randomWalk = False):
        """
