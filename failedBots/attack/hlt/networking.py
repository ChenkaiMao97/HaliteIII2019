...
class Game:
    ...
    def __init__(self):
        ...
        self.game_map = GameMap._generate(self)
    ...
    def update_frame(self):
        ...
        self.game_map.myStartPositions = {}
        for player in self.players.values():
            if player != self.me:
                for ship in player.get_ships():
                    self.game_map[ship.position].mark_unsafe(ship)
            else:
                if len(player.get_ships())>=1:
                    for ship in player.get_ships():
                        self.game_map.aveShipCargo += ship.halite_amount
                        self.game_map.myStartPositions[ship.position] = copy.copy(ship)
                    self.game_map.aveShipCargo /= len(player.get_ships()) 
